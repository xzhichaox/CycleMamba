...
class CycleMamba1Model(BaseModel):
    @staticmethod
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            parser.set_defaults(pool_size=0, gan_mode='vanilla')            # , direction='BtoA'
            parser.add_argument('--lambda_L1', type=float, default=100.0, help='weight for L1 loss')
            # parser.add_argument('--lambda_Se', type=float, default=10.0, help='weight for sematic loss')
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['G_GAN', 'G_L1', 'D_real', 'D_fake','SSIM','Semantic', 'All']    #
        self.visual_names = ['real_A', 'fake_B', 'real_B']
        self.loss_and_weights = [
                                'self.loss_G = self.loss_G_GAN + self.loss_G_L1+ self.loss_SSIM',#+self.loss_semantic  
                                'self.loss_G_L1 = self.criterionL1(self.fake_B, self.real_B) * self.opt.lambda_L1       100',
                                'self.loss_Semantic = self.semantic.calculate_loss(self.fake_B, self.real_B)* self.opt.lambda_L1 ',
                                'self.loss_SSIM = self.ssim(self.fake_B, self.real_B)*10* self.opt.lambda_L1'
                                ]
        if self.isTrain:
    ...
    def backward_G(self):
        ...
        self.loss_SSIM = self.ssim(self.fake_B, self.real_B)* self.opt.lambda_L1
        self.loss_Semantic = self.semantic.calculate_loss(self.fake_B, self.real_B)* self.opt.lambda_L1
        # all loss 
        self.loss_G = self.loss_G_GAN + self.loss_G_L1 + self.loss_SSIM +self.loss_Semantic
